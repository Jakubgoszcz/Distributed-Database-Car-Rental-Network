...
CREATE SEQUENCE seq_klient START WITH 8 INCREMENT BY 1;

CREATE SEQUENCE seq_pracownik START WITH 8 INCREMENT BY 1;

CREATE SEQUENCE seq_samochod START WITH 8 INCREMENT BY 1;

CREATE SEQUENCE seq_wypozyczenie START WITH 8 INCREMENT BY 1;

CREATE SEQUENCE seq_typ_samochodu START WITH 8 INCREMENT BY 1;
...
BEGIN
     IF p_numer_telefonu < 0 THEN
        RAISE_APPLICATION_ERROR(-20001, 'Numer telefonu nie mo?e by? ujemny.');
    END IF;
    
    IF p_pensja < 0 THEN
        RAISE_APPLICATION_ERROR(-20002, 'Pensja nie mo?e by? ujemna.');
    END IF;
...
CREATE OR REPLACE PACKAGE BODY Uzytkownik AS
    ...
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
            RAISE_APPLICATION_ERROR(-20001, 'Nie znaleziono wypo?ycze? dla klienta o ID: ' || p_klient_id);
        WHEN OTHERS THEN
            RAISE;
...
BEGIN
    ...
    IF p_rok_produkcji < 0 OR p_przebieg < 0 OR p_pojemnosc_silnika < 0 OR p_stan_paliwa < 0 OR
       p_cena_za_dzien < 0 OR p_liczba_miejsc < 0 OR p_status < 0 THEN
        RAISE_APPLICATION_ERROR(-20001, 'Warto?ci numeryczne nie mog? by? ujemne.');
    END IF;
...
BEGIN
    IF p_data_zwrotu < p_data_wypozyczenia THEN
        RAISE_APPLICATION_ERROR(-20007, 'Data zwrotu nie mo?e by? wcze?niejsza ni? data wypo?yczenia.');
    END IF;
...
BEGIN
    Pracownik.DodajKlienta('Piotr', 'Wi?niewski', 'Miodowa', '22', '7', 'Gda?sk', '80000', 'Polska', 234567890, 'ID23456789', 'DL234567');
END;
...
BEGIN
    Pracownik.DodajKlienta('Micha?', 'Mazur', 'D?uga', '40', '5', 'Pozna?', '60000', 'Polska', 456789012, 'ID45678901', 'DL456789');
END;
...
BEGIN
    Pracownik.DodajSamochod('Toyota', 'Yaris', 2020, 'WE12345', 8, 'Czerwony', 15000, 1300, 50, 100, 5, 1);
END;


BEGIN
    Pracownik.DodajSamochod('Honda', 'Civic', 2018, 'WZ67890', 9, 'Niebieski', 12000, 1600, 60, 120, 5, 1);
END;



BEGIN
    Pracownik.DodajSamochod('Ford', 'Focus', 2019, 'DW54321', 10, 'Zielony', 10000, 1500, 55, 110, 5, 1);
END;


BEGIN
    Pracownik.DodajSamochod('Volkswagen', 'Golf', 2021, 'KR98765', 11, 'Bia?y', 8000, 1400, 45, 90, 5, 1);
END;
...
BEGIN
    Szef.DodajPracownika(
        'Piotr', 'Wi?niewski', DATE '2023-03-01', NULL,
        'S?oneczna', '7', '16', 'Gda?sk', '80090', 'Polska', 567890123,
...
BEGIN
    Pracownik.ZakonczWypozyczenie(p_id_wypozyczenia => 2); 
END;
...
SET SERVEROUTPUT ON;
EXEC Szef.ZwolnijPracownika(p_pracownik_id => 3); 
...
SET SERVEROUTPUT ON;
EXEC Szef.ZmienStanowiskoPracownika(p_pracownik_id => 1, p_nowe_stanowisko => 'Konsultant');
...
CREATE USER Szef IDENTIFIED BY 12345;
CREATE USER Sprzedawca IDENTIFIED BY 12345;
CREATE USER Klient IDENTIFIED BY 12345;
...
GRANT EXECUTE ON Pracownik TO pracownik;
GRANT EXECUTE ON Pracownik TO uzytkownik;

GRANT EXECUTE ON Uzytkownik TO uzytkownik;


GRANT admin TO Szef;
GRANT pracownik TO Sprzedawca;
GRANT uzytkownik TO Klient;



CREATE OR REPLACE TYPE wypozyczenie_typ AS OBJECT (
    id_wypozyczenia NUMBER,
    klient_id NUMBER,
    samochod_id NUMBER,
    pracownik_id NUMBER,
...
CREATE OR REPLACE TYPE wypozyczenie_tab_typ AS TABLE OF wypozyczenie_typ;

CREATE OR REPLACE FUNCTION pobierz_wszystkie_wypozyczenia
RETURN wypozyczenie_tab_typ PIPELINED IS
BEGIN
    FOR r IN (
        SELECT 
            w.id_wypozyczenia,
            DEREF(w.klient_id).klient_id AS klient_id,
            DEREF(w.samochod_id).samochod_id AS samochod_id,
            DEREF(w.pracownik_id).pracownik_id AS pracownik_id,
            w.data_wypozyczenia,
            w.data_zwrotu,
            w.cena,
            w.status
...
DROP TYPE wypozyczenie_typ;
DROP TYPE wypozyczenie_tab_typ;
DROP FUNCTION pobierz_wszystkie_wypozyczenia;

SELECT * FROM TABLE(pobierz_wszystkie_wypozyczenia);


CREATE OR REPLACE TYPE t_samochod_typ AS OBJECT (
    samochod_id NUMBER,
    marka VARCHAR2(20),
    model_samochodu VARCHAR2(30),
    rok_produkcji NUMBER(4),
...
CREATE OR REPLACE TYPE t_samochod_tab_typ AS TABLE OF t_samochod_typ;

CREATE OR REPLACE FUNCTION pobierz_wszystkie_samochody
RETURN t_samochod_tab_typ PIPELINED IS
BEGIN
    FOR r IN (
        SELECT 
            s.samochod_id,
...
SELECT * FROM TABLE(pobierz_wszystkie_samochody);


CREATE OR REPLACE TYPE klient_typ AS OBJECT (
    klient_id NUMBER,
    imie VARCHAR2(30),
    nazwisko VARCHAR2(30),
    id VARCHAR(10),
...
CREATE OR REPLACE TYPE klient_tab_typ AS TABLE OF klient_typ;

CREATE OR REPLACE FUNCTION pobierz_wszystkich_klientow
RETURN klient_tab_typ PIPELINED IS
BEGIN
    FOR r IN (
        SELECT 
            k.klient_id,
...
SELECT * FROM TABLE(pobierz_wszystkich_klientow);
CREATE OR REPLACE TYPE pracownik_typ AS OBJECT (
    pracownik_id NUMBER,
    imie VARCHAR2(15),
    nazwisko VARCHAR2(15),
    data_zatrudnienia DATE,
...
CREATE OR REPLACE TYPE pracownik_tab_typ AS TABLE OF pracownik_typ;

CREATE OR REPLACE FUNCTION pobierz_wszystkich_pracownikow
RETURN pracownik_tab_typ PIPELINED IS
BEGIN
    FOR r IN (
        SELECT 
            p.pracownik_id,
...
SELECT * FROM TABLE(pobierz_wszystkich_pracownikow);
